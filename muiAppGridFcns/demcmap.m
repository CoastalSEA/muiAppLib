function [cmap,clim0] = demcmap(varargin)
...
%   modified by IHT 27Feb25 to remove external dependencies by adding the
%   function interpcmap
...
elseif nsea <= nseamat
    ...
    hsvsea = interpcmap(hsvseamat,nsea);
end
...
elseif nland <= nlandmat
    ...
    % Linear interpolation (in HSV)
    hsvland = interpcmap(hsvlandmat,nland);
end
...
function hsv = interpcmap(hsvmat,nint)
% function added to replace map.graphics.internal.interpcmap
xq = 1:nint;
nrows = size(hsvmat,1);
x = linspace(1,nint,nrows);
hsv = zeros(nint,3);
for i=1:3
    hsv(:,i) = interp1(x,hsvmat(:,i)',xq,'linear','extrap');
end
